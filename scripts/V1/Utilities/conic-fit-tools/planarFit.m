...
%A class for executing and managing 3D plane fits
    ...
    methods
        ...
        function XYZ=unproject3D(obj,xy,type)
        %Undo the projection operation performaed with project2D().
        %
        %    XYZ=obj.unproject3D(xy,type)
        %
        %in:
        %
        %   xy: A 2xN matrix of 3D coordinates.
        %
        %   type: A string flag, either 'position' (the default) or
        %        'direction'. 
        %        With type='position', the xy data are assumed
        %        to be the positions of points relative to the 2D origin.
        %        Otherwise, xy are assumed to be 2D direction vectors, with
        %        no specific location
        %
        %out:
        %
        %  XYZ: A 3xN matrix of projected 2D coordinates.
