...
% fun_plot.plotTTL(TTLinfo(0.5e6:2e6,:))
% fun_plot.plotTTL(TTLinfo(500000:510000,:))
...
% This section attempts to validate and correct frame timing irregularities.
%
% TWO POSSIBLE PROCESSING PATHS:
% 1. WITH IQ/RF DATA (try block):
%    - Reads precise timestamps directly from raw acquisition files
%    - Identifies frames with timing irregularities (>10ms jitter)
%    - Filters out problematic frames (~5-10% typically)
%    - Provides most accurate timing for hemodynamic analysis
%
% 2. WITHOUT IQ/RF DATA (catch block):
%    - Uses TTL hardware timing (excellent ~0.2ms precision)
%    - Keeps all frames (no filtering)
%    - Standard path when IQ data has been deleted post-processing
%
% BACKGROUND:
% Raw IQ/RF files are the original ultrasound acquisition data before 
% beamforming into PDI. They are typically:
% - Very large (10-100 GB per session)
% - Deleted after PDI conversion to save disk space
% - Only retained for special timing analysis or reprocessing
%
% The catch block is NOT an error - it's the expected path for most users.
...
try
    ...
    warning('off', 'all');
    evalc('[T_pdi_intended, timeTagsSec] = LagAnalysisFusi(fusDatapath);');
    warning('on', 'all');
    ...
    % If range of intervals in a window > 10ms, mark that frame as problematic
    ...
    % 
    % FALLBACK STRATEGY: Use TTL hardware timing
    % - TTL provides excellent temporal precision (~0.2ms resolution)
    % - All frames retained (no filtering)
    % - Adequate for virtually all functional imaging analyses
    % ================================================================
    
    fprintf('\n');
    fprintf('=================================================================\n');
    fprintf('Lag Analysis Note:\n');
    fprintf('  IQ/RF binary files not found in FUSI_data directory.\n');
